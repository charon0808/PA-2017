...
int NDL_OpenDisplay(int w, int h) {
    //printf("In NDL_OpenDisplay, canvas=%d\n",canvas);
  if (!canvas) {
    NDL_CloseDisplay();
  }
  canvas_w = w;
...
int NDL_Render() {
   // printf("In NDL_Render\n");
  if (has_nwm) {
    fflush(stdout);
  } else {
    for (int i = 0; i < canvas_h; i ++) {
      fseek(fbdev, ((i + pad_y) * screen_w + pad_x) * sizeof(uint32_t), SEEK_SET);
     // /**/printf  ("In NDL_Render, fbdev=%d, offset=%d\n",fbdev,((i+pad_y)*screen_w+pad_x)*4);
