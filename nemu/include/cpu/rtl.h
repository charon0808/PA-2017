...
static inline void rtl_sext(rtlreg_t* dest, const rtlreg_t* src1, int width) {
    ...
    switch(width){
        case 1:{
                   //(*dest)|=((*src1)&0x00000080)?0xffffff00:0x0;
                   if ((*src1)&0x00000080){
                       (*dest)=(*src1)|0xffffff00;
                   }
                   else (*dest)=(*src1)&0x000000ff;
                   break;
               }
        case 2:{
                   //(*dest)|=((*src1)&0x00008000)?0xffff0000:0x0;
                   if ((*src1)&0x00008000){
                       (*dest)=(*src1)|0xffff0000;
                   }
                   else (*dest)=(*src1)&0x0000ffff;
                   break;
               }
        default: *dest=*src1;
    }
