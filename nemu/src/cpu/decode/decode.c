...
static inline make_DopHelper(SI) {
     ...
     */
    //TODO();
    op->simm=instr_fetch(eip,id_src->width);
...
make_DHelper(call)   /*modified*/
{
   decode_op_I(eip,id_src,true);
}

make_DHelper(push_width_8)   /*modified*/
{
    id_dest->width=1;
    decode_op_I(eip,id_dest,true);
}

make_DHelper(I_width_8)
{
    id_src->width=1;
    decode_op_I(eip,id_src,true);
}

make_DHelper(I_width_16)
{
    id_src->width=2;
    decode_op_I(eip,id_src,true);
}

make_DHelper(I_width_32)
{
    id_src->width=decoding.is_operand_size_16?2:4;
    decode_op_I(eip,id_src,true);
}

make_DHelper(and_0x25){
    if (decoding.is_operand_size_16){
        id_src->width=id_dest->width=2;
        decode_op_I(eip,id_src,true);
    }
    else {
        id_src->width=id_dest->width=4;
        decode_op_I(eip,id_src,true);
    }
    id_dest->type=OP_TYPE_REG;
    id_dest->reg=0;
...
make_DHelper(pop)  /*modified*/
{
    decode_op_r(eip,id_dest,true);
}

make_DHelper(xor)     /*modified*/
{
   decode_op_rm(eip,id_src,true,id_dest,true);
}

make_DHelper(ret)     /*modified*/
{
   return;
}

make_DHelper(dec)    /*modified*/
{
    return;
}

make_DHelper(xchg)  /*modified*/
{
    decode_op_r(eip,id_dest,true);
}

make_DHelper(rm2r)  /*modified*/
{
    decode_op_rm(eip,id_src,true,id_dest,false);
    decode_op_r(eip,id_dest,true);
}

make_DHelper(r2rm)   /*modified*/
{
    decode_op_r(eip,id_dest,true);
    decode_op_rm(eip,id_src,true,id_dest,false);
}
...
make_DHelper(set)    /*modified*/
{
    decode_op_rm(eip,id_dest,true,NULL,false);
}

make_DHelper(jcc)   /*modified*/
{
    decode_op_r(eip,id_dest,true);
    id_src->width=1;
    decode_op_I(eip,id_src,true);
...
make_DHelper(test)
{
    uint8_t my_code=decoding.opcode&0xff;
    if (my_code==0xf6){
        id_src->width=1;
    }
    else id_src->width=decoding.is_operand_size_16?2:4;
    decode_op_I(eip,id_src,true);
}

make_DHelper(or)
{
    id_src->width=id_dest->width=decoding.is_operand_size_16?2:4;
    if ((decoding.opcode&0xff)==0x0c){
