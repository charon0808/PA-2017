...
void difftest_step(uint32_t eip) {
  ...
  for (int i=0;i<8;i++)
  {
      if (r.array[i]!=cpu.gpr[i]._32){
          diff=true;
          char* my_reg[8]={"eax","ecx","edx","ebx","esp","ebp","esi","edi"};
          printf("Wrong! eip%x  reg %s  QUME: %d   NEMU: %d\n",eip,my_reg[i],r.array[i],cpu.gpr[i]._32);
      }
  }
  if (r.eip!=cpu.eip){
      printf("Wrong! diff in eip, eip of QEMU: 0x%x NEMU: 0x%x\n",r.eip,cpu.eip);
  }
  if (diff) {
    nemu_state = NEMU_END;
  }
